...
class NaiveBayesClassifier:
    ...
    def fit(self, texts, labels):
        ...
        for cls in class_counts:
            ...
            # Laplace-smoothed floor for words not in vocabulary (count=0)
            self.word_log_likelihood[cls]["__UNKNOWN__"] = float(
                np.log(self.alpha / (total_words + self.alpha * vocab_size))
            )
        ...
        for cls in self.class_log_prior:
            ...
            for word in tokens:
                if word in self.word_log_likelihood[cls]:
                    ...
                    # Unknown word: use Laplace-smoothed floor probability
                    score += self.word_log_likelihood[cls]["__UNKNOWN__"]
            scores[cls] = score
...
class EnsembleModel:
    ...
    def train(self):
        ...
        train_df = pd.read_csv("dataset/expanded_dataset.csv")
        test_df = pd.read_csv("dataset/benchmark_dataset.csv")
